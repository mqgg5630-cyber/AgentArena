...
- 根因/思路：两个行为叠加——(1) `watch.ps1` **每次 poll 一开始就从当前 worktree 读配置**，之后才 `sync.ps1`（pull）；(2) 没有等待中的请求时 poll 在 `exit 0` 处直接返回，**根本不会 pull**。于是"空闲轮不更新、处理请求的那一轮用旧配置"——配置永远滞后一轮生效，日志里记录的 `cmd:` 恰好是我们唯一的证据。
- 解法：不要在这一轮里找代码问题，直接发下一轮请求即可（本机 worktree 在处理上一轮时已被同步）。同时把这条写进协议文档 §10.3.2，并在 v2.4.7（总部已发）里新增 `elapsed:` 与"空输出显式标注"，让"0 秒 + 零输出"的假通过一眼可辨。
- 教训/可复用点：[通用] 判断"改动有没有生效"时，**先确认执行方读到的是哪个版本的配置**；轮询型执行器常把"读取配置"与"同步代码"分开（甚至只在有活时才同步），日志里回显的原始命令字符串比任何推断都可靠。别把这种滞后当成"改动没起作用"而去改代码。
